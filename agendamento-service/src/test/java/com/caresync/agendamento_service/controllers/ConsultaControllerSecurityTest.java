...
class ConsultaControllerSecurityTest {
    ...
    void medico_consegueEditarConsulta() throws Exception {
        ...
        ResponseEntity<String> createResponse = restTemplate
                .withBasicAuth("medico1", "123456")
                .postForEntity(url("/consultas"), new HttpEntity<>(createBody, headers), String.class);

        assertThat(createResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
        String consultaId = objectMapper.readTree(createResponse.getBody()).path("id").asText();
    ...
    @Test
    void paciente_naoConseguePatchConsulta_retorna403() {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        String body = """
                {"status": "CANCELADA"}
        ...
        ResponseEntity<String> response = restTemplate
                .withBasicAuth("paciente1", "123456")
                .exchange(url("/consultas/1"), org.springframework.http.HttpMethod.PATCH,
                        new HttpEntity<>(body, headers), String.class);
    ...
    @Test
        void medico_podeAtualizarConsultaViaMutationGraphQL() {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        String body = """
                {"query": "mutation { atualizarConsulta(id: 1, status: \\"CANCELADA\\") { id status } }"}
                """;
    ...
    @Test
    void paciente_naoConsegueAtualizarConsultaViaMutationGraphQL() {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        String body = """
                {"query": "mutation { atualizarConsulta(id: 1, status: \\"CANCELADA\\") { id status } }"}
